...
electricity meters. jMBus is licensed under the Mozilla Public License
(MPL) v.2.0.
...
rom Radiocrafts. The jMBus library only supports passive listening for
messages in modes S and T at the moment. Encryption mode 5 (AES with
CBC and IV) is supported.
...
One command line applications is part of the library. You can
execute this using the script found in the folder "run-scripts". The
file ending with .sh is a shell script for Unix/Linux/Mac. The file
ending with .bat.winfile is a batch file for use with Windows. In order
to run the script in Windows you have to rename the batch file by
removing the ".winfile" ending so that it simply end with ".bat".

jMBus contains the following command line applications:

* *jmbusApp* - can be used to:
